...
describe("transitionSession", () => {
  ...
  it("moves DRIVE_READY to READY on inspect connect", () => {
    expect(transitionSession(SessionState.DRIVE_READY, "INSPECT_CONNECTED")).toBe(
      SessionState.READY
    );
  });

  it("moves INSPECT_READY to READY on drive connect", () => {
    expect(transitionSession(SessionState.INSPECT_READY, "DRIVE_CONNECTED")).toBe(
      SessionState.READY
    );
  });
...
describe("retry rules", () => {
  ...
  it("retries inspect ops once when reconnect succeeds", () => {
    expect(
      shouldRetryInspectOp({ attempt: 0, reconnectSucceeded: true })
    ).toBe(true);
    expect(
      shouldRetryInspectOp({ attempt: 1, reconnectSucceeded: true })
    ).toBe(false);
    expect(
