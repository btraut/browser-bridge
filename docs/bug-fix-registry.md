...
- `snapshot-ref-metadata-fallback` - Snapshot refs now persist link/name/role metadata in the page so `locator.ref` can recover after list rerenders instead of failing the moment the original `data-bv-ref` attribute disappears. Ref: pending commit
