...
- `popup-trigger-click-state-guard` - Popup/menu trigger clicks now carry trigger state into the background CDP path and return `FAILED_PRECONDITION` when they only focus the control without opening it. Ref: pending commit
