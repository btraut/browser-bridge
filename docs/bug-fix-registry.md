...
- `cli-bin-shebang` - The CLI source entrypoint now carries the Node shebang and test coverage checks the packaged bin target, so `tsc`-built installs do not regress into raw-JavaScript shell execution. Ref: pending commit
