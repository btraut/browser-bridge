...
- `session-tab-affinity` - Omitted-tab drive actions now return the resolved tab target and core stores it as the session primary tab so later actions stay pinned to the same browser context. Ref: pending commit
