...
- `popup-trigger-click-state-guard` - Popup/menu trigger clicks now return `FAILED_PRECONDITION` when they only focus the control without changing open state, instead of reporting a false-positive success. Ref: pending commit
