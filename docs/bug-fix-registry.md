...
- `inspect-explicit-tab-targeting` - Inspect routes now default to the session primary tab, still honor explicit `tab_id` hints end-to-end, and `drive.tab_activate` degrades focus verification failures to warnings once the requested tab is active. Ref: pending commit
