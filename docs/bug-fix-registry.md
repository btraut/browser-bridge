...
- `interactive-ax-menu-roles` - AX snapshot interactive filtering now keeps `menu`, `menuitem`, `menuitemcheckbox`, and `menuitemradio` nodes so open overlay actions remain visible in `inspect.dom_snapshot`. Ref: pending commit
